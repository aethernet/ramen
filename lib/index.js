...
var undoable__default = _interopDefault(undoable);
var Selection = _interopDefault(require('@simonwep/selection-js'));
...
const EditorBackground = styled__default.div `
...
function Background(props) {
    const editorId = reactRedux.useSelector((state) => state.references.editorId);
    return (React.createElement(EditorBackground, Object.assign({}, props, { id: editorId })));
}
var EditorBackground$1 = React.memo(Background);
...
function EditorEvents() {
    ...
    const pendingConnectionOrigin = reactRedux.useSelector(getPendingConnection);
    const editorId = reactRedux.useSelector((state) => state.references.editorId);
    React.useEffect(() => {
...
const Viewport = styled__default.div `
  height: 100%;
  width: 100%;
  overflow: hidden;
`;
const EditorWrapperBackground = styled__default.div.attrs(({ xPos, yPos, scale }) => ({
    style: {
...
function usePanning(canPan, viewport) {
    const dispatch = reactRedux.useDispatch();
    const isPanning = reactRedux.useSelector((state) => (state.viewport.isPanning), reactRedux.shallowEqual);
    React.useEffect(() => {
        if (!viewport || !canPan)
            return;
...
function useZooming(canZoom, viewport) {
    const dispatch = reactRedux.useDispatch();
    React.useEffect(() => {
        if (!viewport || !canZoom)
            return;
...
function EditorWrapper(props) {
    const { xPos, yPos, zoom } = reactRedux.useSelector(getViewport, reactRedux.shallowEqual);
...
function EditorViewport(props) {
    ...
    const viewportId = reactRedux.useSelector(getViewportId, reactRedux.shallowEqual);
...
const StyledInput = styled__default.input `
  ...
  font-size: .9rem;
`;
const ControlBackground = styled__default.div `
  display: flex;
  height: 100%;
  overflow: hidden;
  span {
...
function DefaultControl(props) {
    ...
    return (React.createElement(ControlBackground, { className: "noDrag" },
        React.createElement(StyledInput, Object.assign({ onClick: onInputClick, placeholder: name }, rest))));
}

const PinContainer = styled__default.div `
  ...
  align-self: center;
`;
const Pin = styled__default(PinContainer) `
...
const InputPin = styled__default(Pin) `
  transform: ${getTransform({ isInput: true })};
`;
const OutputPin = styled__default(Pin) `
  transform: ${getTransform({ isInput: false })};
`;
const Background$1 = styled__default.div `
  ...
  color: #96A1A9;
`;
const FieldContent = styled__default.div `
  overflow: hidden;
  display: flex;
  flex: 1;
  font-size: 1rem;
...
`;
const NodeWrapper = styled__default.div `
  display: inline-block;
...
`;
const NodeTitle = styled__default.div `
  padding: 12px 8px;
  font-size: 1.1rem;
  height: 36px;
  text-align: left;
...
`;
const NodeSubtitle = styled__default.div `
  color: ${({ theme }) => theme.textSecondary || lightTheme.textSecondary};
  font-size: .9rem;
  text-align: left;
`;
const ControlWrapper = styled__default.span `
  display: flex;
  max-width: 100%;
`;
...
function NodeField(props) {
    ...
    const fieldDataTypeDetails = reactRedux.useSelector((state) => getDataType(state, dataType), reactRedux.shallowEqual);
    const dataTypeControlProps = reactRedux.useSelector((state) => getControlProps(state, dataType), reactRedux.shallowEqual);
    const hasInputConnection = reactRedux.useSelector((state) => isFieldInputConnected(state, nodeId, fieldId), reactRedux.shallowEqual);
    ...
    return (React.createElement(DefaultField, { nodeId: nodeId, fieldId: fieldId, color: fieldDataTypeDetails.color, key: fieldId, input: hasInput, output: hasOutput, height: FIELD_HEIGHT }, hideControl ? fieldName :
        React.createElement(ControlWrapper, { className: "noDrag" },
            React.createElement(FieldControl, Object.assign({ name: fieldName }, customControlProps)))));
}
...
function DefaultNode(props) {
    ...
    const nodeType = reactRedux.useSelector((state) => getNodeType(state, type));
    const dispatch = reactRedux.useDispatch();
    const typeName = nodeType.name;
    const nodeWidth = nodeType.width || NODE_WIDTH;
    const nodeFields = get(nodeType, `fields`, [])
        .map((field) => (React.createElement(MemoizedNodeField, { key: field.id, fieldId: field.id, nodeId: nodeId, dataType: field.dataType, controlType: field.controlType, controlProps: field.controlProps, fieldProps: nodeState[field.id], controls: controls, hideControlOnInput: field.hideControlOnInput, name: field.name, hasInput: field.input, hasOutput: field.output })));
    return (React.createElement(NodeWrapper, Object.assign({}, rest, { id: nodeId, className: `${NODE_CLASSNAME} ${className || ""}`, style: style, width: nodeWidth, selected: selected, onPointerDown: () => {
            if (!selected)
                dispatch(setSelection([nodeId]));
        } }),
        React.createElement(NodeTitle, null,
            name || typeName,
            name && React.createElement(NodeSubtitle, null, typeName)),
        React.createElement("div", null, nodeFields)));
}
var DefaultNode$1 = React.memo(DefaultNode, (prevProps, nextProps) => {
...
function DragWrapper(props) {
    ...
    const dispatch = reactRedux.useDispatch();
    const zoom = reactRedux.useSelector(getViewportZoom);
    const editorId = reactRedux.useSelector(getEditorId);
    return (React.createElement(Draggable, { scale: zoom, bounds: `#${editorId}`, position: defaultPos, cancel: ".noDrag", onDrag: onDrag, onStop: onStop },
        React.createElement(Wrap, { selected: selected }, children)));
}
var DragWrapper$1 = React.memo(DragWrapper, (prevProps, nextProps) => {
...
function NodeLayer(props) {
    const nodes = reactRedux.useSelector(getNodes, reactRedux.shallowEqual);
    const selection = reactRedux.useSelector(getSelection, reactRedux.shallowEqual);
    const dispatch = reactRedux.useDispatch();
    return (React.createElement("div", { style: { position: "relative", height: "100%", width: "100%" } }, nodes.map((node) => (React.createElement(DragWrapper$1, { selected: selection[`${node.id}`], key: node.id, nodeId: node.id, defaultX: node.x, defaultY: node.y, onDrag: (e, { deltaX, deltaY }) => {
            dispatch(dragNodes(Object.keys(selection), { x: deltaX, y: deltaY }));
        }, onStop: (e, { x, y }) => {
            dispatch(dropNode(node.id, { x, y }));
        } },
        React.createElement(DefaultNode$1, { selected: selection[`${node.id}`], nodeId: node.id, name: node.name, type: node.type, controls: props.controls, nodeState: (props.graphState || {})[`${node.id}`] }))))));
}
...
const NoodlePath = styled__default.path `
...
const Noodles = styled__default.svg `
  position: absolute;
  z-index: 90;
...
function NoodleWrapper(props) {
    ...
    const dispatch = reactRedux.useDispatch();
    const connectionStart = reactRedux.useSelector((state) => getConnectionStart(state, connection));
    const connectionEnd = reactRedux.useSelector((state) => getConnectionEnd(state, connection));
...
function PendingConnection(props) {
    const { pendingConnectionEndPos, pendingConnectionOrigin, } = reactRedux.useSelector(getEditor, reactRedux.shallowEqual);
    const pendingConnectionStartPos = reactRedux.useSelector((state) => getConnectionStart(state, pendingConnectionOrigin), reactRedux.shallowEqual);
    if (!pendingConnectionOrigin)
        return null;
...
function NoodleLayer() {
    const connections = reactRedux.useSelector(getConnections, reactRedux.shallowEqual);
...
const eventsMiddleware = (events) => (store) => (next) => (action) => {
    ...
    if (type === CREATE_CONNECTION ||
        ...
        type === undoable.ActionCreators.undo() ||
        type === undoable.ActionCreators.redo() ||
        type === DROP_NODE) {
...
const undoableReducers = redux.combineReducers({
...
const rootReducer = redux.combineReducers({
    ...
    history: undoable__default(undoableReducers, {
        ignoreInitialState: true,
        groupBy: undoable.groupByActionTypes([SET_NODES, SET_CONNECTIONS]),
        filter: undoable.excludeAction([
            INIT_NODES,
            INIT_CONNECTIONS,
            DRAG_NODES,
            SET_SCHEMA,
...
function configureStore(preloadedState, events) {
    ...
    const store = redux.createStore(rootReducer, preloadedState, composedEnhancers);
    return store;
}
...
function Ramen(props) {
    ...
    return (React.createElement(RamenProvider, { onSelection: onSelection, onGraphChange: onGraphChange, onConnectionCreate: onConnectionCreate, onConnectionDelete: onConnectionDelete, onNodePositionChange: onNodePositionChange, initialEditorState: initialEditorState, initialGraph: initialGraph, schema: schema, graph: graph },
        React.createElement(EditorViewport$1, { canZoom: canZoom, canPan: canPan },
            React.createElement(EditorBackground$1, { height: height, width: width },
                children,
                React.createElement(NoodleLayer, null),
                React.createElement(NodeLayer, { controls: controls, graphState: graphState }),
                React.createElement(EditorEvents, null),
